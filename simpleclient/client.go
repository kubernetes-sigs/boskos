...
//
// For a variant of this call which accepts a password file see [sigs.k8s.boskos/fileclient.NewClient].
func NewClient(owner string, urlString, username string, passwordGetter func() []byte) (*Client, error) {
	if (username == "") != (passwordGetter == nil) {
		return nil, fmt.Errorf("username and passwordGetter must be specified together")
	}
...
func (c *Client) ReleaseAll(dest string) error {
	...
	for _, r := range resources {
		...
		if err != nil {
			allErrors = appendErr(allErrors, err)
		}
	}
...
func (c *Client) acquire(rtype, state, dest, requestID string) (*common.Resource, error) {
	...
	work := func(retriedErrs *[]error) (bool, error) {
		...
		if err != nil {
			...
			*retriedErrs = appendErr(*retriedErrs, err)
			return false, nil
		}
		...
		default:
			*retriedErrs = appendErr(*retriedErrs, fmt.Errorf("status %s, status code %v", resp.Status, resp.StatusCode))
...
func (c *Client) Release(name, dest string) error {
	...
	work := func(retriedErrs *[]error) (bool, error) {
		...
		if resp.StatusCode != http.StatusOK {
			*retriedErrs = appendErr(*retriedErrs, fmt.Errorf("status %s, statusCode %v releasing %s", resp.Status, resp.StatusCode, name))
			return false, nil
		}
		return true, nil
	}
...
func (c *Client) Update(name, state string, userData *common.UserData) error {
	...
	work := func(retriedErrs *[]error) (bool, error) {
		...
		if resp.StatusCode != http.StatusOK {
			*retriedErrs = appendErr(*retriedErrs, fmt.Errorf("status %s, status code %v updating %s", resp.Status, resp.StatusCode, name))
			return false, nil
		}
		return true, nil
	}
...
func retry(work workFunc) error {
	...
	return errors.Join(retriedErrs...)
}

// appendErr preserves the multierror.Append semantic of
// de-duplicating errors: if the previous error has the same
// string representation, the new one doesn't get appended.
func appendErr(errs []error, err error) []error {
	if len(errs) > 0 && errs[len(errs)-1].Error() == err.Error() {
		return errs
	}
